...
const _safeReverseGeocode = async (lat, lng) => {
  ...
  const fallback = { locality: 'Unknown', city: 'Unknown', state: 'Unknown' };

  try {
    const response = await fetch(
      `https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lng}&addressdetails=1`,
      { headers: { 'User-Agent': 'ZugadoApp/1.0', 'Accept': 'application/json' } },
    );
    ...
    const result = {
      locality: address.suburb || address.neighbourhood || address.road || 'Unknown',
      city: address.city || address.town || address.village || 'Unknown',
      state: address.state || 'Unknown',
      postcode: address.postcode || address.pincode || address.postal_code || '',
    };
